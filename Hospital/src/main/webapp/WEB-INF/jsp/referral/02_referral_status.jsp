...
.table-controls .left{ display:flex; align-items:center; gap:8px; }
...
.table-controls .right .meta{ font-size:13px; color:#555; }
...
.data-table{
  ...
  border-spacing:0;
  min-width:720px; /* 컬럼이 많은 경우 깨짐 방지 */
}
    ...
    <div class="content">
      ...
      <div class="table-controls">
        <div class="left">
          ...
          <select id="pageSizeSelect" aria-label="페이지당 표시 개수">
          ...
          <tbody>
            <c:forEach var="request" items="${requests}" varStatus="status">
              <tr>
                ...
                <td><fmt:formatDate value="${request.createdAt}" pattern="yyyy-MM-dd"/></td>
                <td>
                  <c:choose>
                    <c:when test="${request.replyExists}">
                      <form action="/referral/02_referral_status_history.do" method="get">
                        <input type="hidden" name="requestId" value="${request.requestId}" />
                        <button type="submit">확인</button>
                      </form>
                    </c:when>
                    <c:otherwise>
                      <form action="/referral/02_referral_status_board.do" method="get">
                        <input type="hidden" name="requestId" value="${request.requestId}" />
